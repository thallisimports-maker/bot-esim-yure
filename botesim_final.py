...
def inicializar_banco():
    ...
    try:
        ...
        if cur.fetchone() == 0: cur.execute("INSERT INTO estoque (produto_id, quantidade) VALUES ('vivo_30gb', 0), ('tim_40gb', 0), ('claro_40gb', 0)")
        con.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    botoes = [[InlineKeyboardButton(f"Vivo 30GB - R$ 25 ({est.get('vivo_30gb', 0)} un)", callback_data="buy_vivo_30gb")], [InlineKeyboardButton(f"Tim 40GB - R$ 30 ({est.get('tim_40gb', 0)} un)", callback_data="buy_tim_40gb")], [InlineKeyboardButton(f"Claro 40GB - R$ 35 ({est.get('claro_40gb', 0)} un)", callback_data="buy_claro_40gb")], [InlineKeyboardButton("➕ Adicionar Saldo (Pix)", url="https://bot-esim-yure.onrender.com")]]
...
async def generar_fluxo_pix(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 🚀 ROTA ATUALIZADA PARA ACEITAR POST E OPTIONS, ACABANDO COM O BLOQUEIO DE PREFLIGHT DO CHROME
@api_app.route("/api/admin/gerar-pix-site", methods=["POST", "OPTIONS"])
async def api_gerar_pix_site(request, response: Response):
    # Trata a requisição de teste secreta do Google Chrome (OPTIONS)
    ...
    # Fluxo normal do POST quando o cliente clica no botão
    dados_corpo = await request.json()
    valor_input = dados_corpo.get("valor", 10.0)
